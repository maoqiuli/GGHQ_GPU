...
#pragma omp parallel for
  ...
  void build(const int part_id, const int shard_id = 0) {
    ...
    // std::cout << "layer_top->layer_btm,t_merge,t_select,t_sym,t_total\n" ;
    // FILE *fp = NULL;
    // if (part_id == 0) {
    //     fp = fopen("./fout/build_time.csv", "w");
    //     fprintf(fp, "layer_top->layer_btm,t_merge,t_select,t_sym,t_total\n"); 
    // }
    for (int layer_top = 0; layer_top < L; layer_top++) {
      for (int layer_btm = layer_top; layer_btm >= 0; layer_btm--) {
        ...
        std::chrono::duration<double> t_merge = (t2 - t1) * 1000;
        std::chrono::duration<double> t_select = (t3 - t2) * 1000;
        std::chrono::duration<double> t_sym = (t4 - t3) * 1000;
        std::chrono::duration<double> t_total = (t4 - t1) * 1000;
        // std::cout << layer_top << "->" << layer_btm << "," << t_merge.count() << "," << t_select.count() << "," << t_sym.count() << "," << t_total.count() << "\n";
        // if (part_id == 0)
        //   fprintf(fp, "%d->%d,%f,%f,%f,%f\n", layer_top, layer_btm, t_merge.count(), t_select.count(), t_sym.count(), t_total.count()); 
      }
    }
    // if (part_id == 0)
    //   fclose(fp);
  ...
  void refine(const int part_id = 0, const int shard_id = 0) {
    // FILE *fp = NULL;
    // if (part_id == 0) fp = fopen("./fout/build_time.csv", "a");
    for (int layer = L - 2; layer >= 0; layer--) {
      ...
      auto t3 = std::chrono::steady_clock::now();
      std::chrono::duration<double> t_merge = (t2 - t1) * 1000;
      std::chrono::duration<double> t_sym = (t3 - t2) * 1000;
      std::chrono::duration<double> t_total = (t3 - t1) * 1000;
      // if (part_id == 0)
      //   fprintf(fp, "%d->%d,%f,%f,%f,%f\n", L - 1, layer, t_merge.count(), 0.0, t_sym.count(), t_total.count()); 
    }
    // if (part_id == 0) fclose(fp);
  }
};
